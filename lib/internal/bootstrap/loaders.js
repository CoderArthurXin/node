...
/**
 * @[ar-1]: 可以通过 process.binding 获取到的 C++ 模块的白名单
 */
...
/**
 * @[ar-1]: 这里也是此loader的一个重要作用，setup process.binding() 和 process._linkedBinding()
 */
...
// Set up process.binding() and process._linkedBinding().
{
  ...
  process.binding = function binding(module) {
    ...
    if (internalBindingAllowlist.has(module)) {
      ...
      return internalBinding(module); // 查找内建C++模块
    }
  ...
  /**
   * @[ar-1]: 通过 process._linkedBinding 时，要先 AddLinkedBinding
   */  
  process._linkedBinding = function _linkedBinding(module) {
    module = String(module);
    let mod = bindingObj[module];

    // getLinkedBinding 也是 BootstrapInternalLoaders 加载本loader时通过参数传进来的
 ...
 */
let internalBinding; // 作用就是找内建C++模块
{
  const bindingObj = ObjectCreate(null);
  ...
  internalBinding = function internalBinding(module) {
    ...
    /**
     * @[ar-1]: bindingObj 是缓存，如果缓存找不到，在去找内建C++
     */    
    if (typeof mod !== 'object') {

      /**
       * @[ar-1]: getInternalBinding 是查找内建C++模块，它是在 BootstrapInternalLoaders 中传进来的
       *   内建C++模块列表可以在 NODE_BUILTIN_STANDARD_MODULES 里找
       */      
...
/**
 * @[ar-1]: native_module 模块就是 src\node_native_module_env.cc
 */
const {
  moduleIds,
  compileFunction
...
const loaderExports = {
  internalBinding,  // 将找内建C++模块的能力透传出去
  NativeModule, 
  require: nativeModuleRequire  // 将找Native js模块的能力透传出去
};
